...
# Based on LSOA-level prescription data and IMD rankings
...
# ============================================================================
# Figure 2: IMD Analysis with Diabetes Measures
# ============================================================================
...
ribplot_diabetes <- ggplot() +
  ...
  labs(x = "Date", y = "Prescribing measure index (Median)", title = "A") +
  theme(legend.position = "right", plot.title = element_text(size = 25)) +
...
# 2B: Percentage difference by decile
...
figure_2_diabetes <- grid.arrange(ribplot_diabetes, bars_IMD_diabetes,
                                  layout_matrix = rbind(c(1, 1, 1, 2, 2),
                                                        c(1, 1, 1, 2, 2),
                                                        c(1, 1, 1, 2, 2)))

ggsave(paste0(output_dir_plots, "figure_2_diabetes.jpg"), 
       plot = figure_2_diabetes, dpi = 300, width = 12, height = 8, units = "in")

# ============================================================================
# Figure 3: Pre/Post Intervention Analysis
# ============================================================================
...
measlines_prepost_diabetes <- ggplot(
  ...
  aes(x = date_period, y = period_avg, group = IMD_decile_rank, linetype = IMD_decile_rank)
) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  ...
  labs(x = "Time Period", y = "Prescribing index aggregated by time period", title = "B") +
  theme(plot.title = element_text(size = 25)) +
  guides(linetype = guide_legend(title = "Deprivation Level", 
                                 title.position = "top",
                                 labels = c("  Most deprived" = "Most deprived",
                                            "  Least deprived" = "Least deprived", 
                                            "  National Median" = "National Median"))) +
  theme(legend.title = element_text(size = 10),
        legend.text = element_text(size = 9))

figure_3_diabetes <- grid.arrange(ribplot_diabetes, measlines_prepost_diabetes,
                                  layout_matrix = rbind(c(1, 1, 1, 2, 2),
                                                        c(1, 1, 1, 2, 2),
                                                        c(1, 1, 1, 2, 2),
                                                        c(1, 1, 1, 2, 2)))
...
cat("- figure_2_diabetes.jpg: IMD inequality analysis\n") 
cat("- figure_3_diabetes.jpg: Pre/post intervention comparison\n")
